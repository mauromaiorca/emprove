...
def transformCtfImage_torch(I, nx, ny, angpix, Voltage, DefocusU, DefocusV, DefocusAngle, SphericalAberration, CtfBfactor, PhaseShift, AmplitudeContrast,  device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")):
    # Convert I to a PyTorch tensor and move to the specified device
    I_tensor = torch.tensor(I, device=device).reshape(ny, nx)
    ctfI_np = emprove_core.CtfCenteredImage(nx, ny, angpix, SphericalAberration, Voltage, DefocusAngle, DefocusU, DefocusV, AmplitudeContrast, CtfBfactor, PhaseShift)
    ctfI = torch.tensor(ctfI_np, device=device).reshape(ny, nx)
    if ctfI.nelement() != nx*ny or I_tensor.nelement() != nx*ny:
        return torch.zeros(nx*ny, device=device).tolist()

    else:
        # FFT operations using PyTorch
        mapFFT = torch.fft.fftshift(torch.fft.fft2(I_tensor))
        mapIFFT = torch.real(torch.fft.ifft2(torch.fft.ifftshift(mapFFT * ctfI))).flatten().tolist()
        return mapIFFT

def transformCtfImage(I,nx,ny,angpix,Voltage,DefocusU, DefocusV, DefocusAngle, SphericalAberration, CtfBfactor, PhaseShift, AmplitudeContrast):
...
def ParticleVsReprojectionScores_GPU(particlesStarFile: PathLike, scoredParticlesStarFile: PathLike, referenceMap: PathLike, referenceMask: PathLike, angpix, listScoresTags = "", numProcesses = 1, numViews=[50,200,400], doCTF=False):
    if listScoresTags == "":
